...
from timm.data import Mixup as Mixup_fn_lib
...
def get_args_parser():
    ...
    # Model params
    parser.add_argument('--arch', default='main_vit_vit', type=str, metavar='ARCH',
...
def main(args):
    ...
    if args.finetune and not args.eval:
        ...
        if not args.is_momentum:
            ...
            for k in list(state_dict.keys()):
                # retain only base_encoder (student) up to 1st layer of decoder layer
                if k.startswith('momentum_encoder') and 'decoder' not in k and 'mask' not in k and 'base_encoder' not in k:
                    # remove prefix
                    state_dict[k[len("momentum_encoder."):]] = state_dict[k]
                del state_dict[k]
            print('Loading momentum model.')
        # interpolate position embedding
        pos_embed.interpolate_pos_embed(model, checkpoint)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats = evaluate(data_loader_val, model, device)
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
        save_best = test_stats["acc1"] >= max_accuracy
        max_accuracy = max(max_accuracy, test_stats["acc1"])
        print(f'Max accuracy: {max_accuracy:.2f}%')

        if args.output_dir and epoch%15==0:
            utils.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, is_best=save_best)
        elif args.output_dir and epoch == (args.epochs - 1):
            utils.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, is_best=save_best)
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    mixup_fn: Optional[Mixup_fn_lib] = None, log_writer=None,
                    args=None):
    model.train(True)
